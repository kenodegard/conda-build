...
from os.path import abspath, basename, expanduser, expandvars, isfile
...
def configure_parser(parser: ArgumentParser | None, **kwargs) -> ArgumentParser:
    ...
    from .main_render import add_parser_render
    ...
    pypi_grp.add_argument(
        ...
        default=(
            abspath(expanduser(expandvars(context.conda_build.get("pypirc"))))
            if context.conda_build.get("pypirc")
            else context.conda_build.get("pypirc")
        ),
    )
    pypi_grp.add_argument(
        "--repository",
    ...
    parser.add_argument(
        ...
        default=(
            abspath(expanduser(expandvars(context.conda_build.get("cache_dir"))))
            if context.conda_build.get("cache_dir")
            else context.conda_build.get("cache_dir")
        ),
    )
    parser.add_argument(
...
@deprecated(
    "24.5",
    "24.7",
    addendum="Use `conda_build.cli.main_build.configure_parser` instead.",
)
...
@deprecated(
    "24.5",
    "24.7",
    addendum="Use `conda_build.cli.main_build.validate_recipe` instead.",
)
def check_recipe(path_list: Iterable[str]) -> None:
...
def validate_recipe(path: str) -> str:
    from ..utils import VALID_METAS

    if isfile(path) and basename(path) in VALID_METAS:
        warnings.warn(
            (
                f"RECIPE_PATH received is a file ({path}).\n"
...
def output_action(recipe: str, config: Config):
    from .. import api
...
def main(args: Sequence[str] | None = None) -> int:
    """For conda-build entrypoint. See conda_build.plugin for plugins."""
    parser = configure_parser(None)
    parsed = parser.parse_args(args)
