...
def combine_specs(specs, log_output=True):
    ...
    errors.extend(
        f"  - (src:{src}) {k!r} key contains an invalid character '-'"
    ...
    errors.extend(
        ...
        if k in keys
        else keys.__setitem__(
            k, zg
        )  # remember the first zip_keys group this key was seen in
        ...
        for key in set(spec).intersection(extend_keys) - {"zip_keys", "extend_keys"}:
            if isinstance(spec[key], dict):
                ...
                if isinstance(prior, dict):
                    ...
                    logshim(
                        f"(src:{src}) {key!r} clobbering a {type(prior).__name__} with dict"
                    )
                ...
                if isinstance(prior, set):
                    ...
                    logshim(
                        f"(src:{src}) {key!r} clobbering a {type(prior).__name__} with set"
                    )
                    combined[key] = set(ensure_list(spec[key]))

    # clobber keys: keys that override prior values
    for src, spec in specs.items():
        logshim(f"(src:{src}) adding variants")
        ...
        for key in set(spec).difference(extend_keys) - {"zip_keys", "extend_keys"}:
            ...
            if all(k in spec for k in zg):
                errors.append(
                    ...
                    + "\n".join(f"      {k!r}: {v}" for k, v in sizes.items())
                )
                continue
            ...
            missing = set(ensure_list(spec[key], include_dict=False)).difference(
                combined.get(key, [])
            )
            if missing:
                errors.append(
                    "  - (src:{}) {}filtering for unimplemented subspace(s) {}".format(
                        src,
                        f"did not fully implement all fields for zip_keys group {tuple(zg)} or "
                        if len(zg) > 1
                        else "",
                        tuple(missing),
                    )
    ...
    errors.extend(
        f"  - (src:{src}) key {k!r} in zip_keys group {tuple(zg)} is not defined in the combined spec"
        for zg, src in zip_keys.items()
    ...
    if errors:
        raise ValueError(f"Variant configuration errors:\n{errors}")
